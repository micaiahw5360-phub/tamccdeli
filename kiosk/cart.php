...
function getOptionDetailsFromIds($conn, $option_ids) {
    ...
    $stmt = $conn->prepare("
        SELECT ov.id, ov.value_name, ov.price_modifier, o.option_name
        FROM menu_item_option_values ov
...
// Handle AJAX updates (quantity increment/decrement, remove)
...
if (!empty($cart)) {
    ...
    foreach ($cart as $key => $entry) {
        ...
        $option_details = getOptionDetailsFromIds($conn, $options);
        $modifier_total = array_sum(array_column($option_details, 'price_modifier'));
